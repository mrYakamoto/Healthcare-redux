...
module ScraperHelper
  ...
  def import_csv_data
    ...
    csv.each do |clinic|
      p clinic
      save_cpc_to_db_with_no_Lat_Lng(clinic)
    end
  ...
  def save_cpc_to_db_with_no_Lat_Lng(clinic)
    name = clinic["name"]
    ...
    clinic_object = Clinic.new(name: name, full_address: full_address, street_address: street_address, city: city, state: state, zip: zip, clinic_type: "CPC")
    p clinic_object
    clinic_object.save!
  end
  ...
  def planned_parenthood_state_page_parser(state_page, state_abbrev)
    ...
    state_page_clinics_div_html.each do |c|
      ...
      Clinic.create!(name:name, full_address:full_address, state:state_abbrev, clinic_type:"PP")
    end
  end
